...
import { getByTestId, getByText, prettyDOM, queryByTestId, queryByTitle, waitFor } from '@testing-library/dom'
...
describe('pages/Profile', () => {
    it('should logout from profile and redirect to onboarding', async () => {
        ...
        await step('wait openning onboarding page', async () => {
            await waitFor(() =>
                expect(queryByTestId(document.body, 'sign-in-screen')).toBeInTheDocument()
            );
        });
